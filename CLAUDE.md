...
### Rahmen v7 -- randlos war der falsche Hebel

Ersetzt `bgl_niche_frame` aus v4. Drei Zwischenversuche, alle durchgerechnet statt nur angeschaut, bevor verworfen:

- **v5** (Einzelsteg, deutlich duenner): Nische zu Rahmen 68 % auf 74 % verbessert -- aber gespreizte Fuesse unter der unteren Leiste haben die automatische Konturmessung auf 100 % der Leinwand aufgeblaeht (siehe naechster Abschnitt zum Pipeline-Fix). Verworfen.
- **v6** (randlos, exakt bis zur Bildkante): brachte einen doppelten Gehrungsrahmen (zwei ineinander verschachtelte Stege mit Diagonalverbindung an den Ecken) statt eines einzelnen Stegs, und netto *weniger* Regalbreite als v4 trotz duennerer Einzelstege. Verworfen.
- **v7** (siehe unten): uebernommen.

**Die randlos-Vorgabe war der eigentliche Fehler, nicht nur v6s Umsetzung davon.** `k = min(H/(frame.h*sourceH), W/(frame.w*sourceW))` -- die Bildschirmhoehe ist 2400 Geraetepixel, die Bildbreite 1236, das Seitenverhaeltnis der Renders liegt aber konstant bei rund 1440x2580 (FRAME_WIDTH ist fix, das Modell liefert zuverlaessig nahe 9:16). Ein Rahmen, der randlos bis zur Bildkante reicht (`frame.h = 1`), nimmt der Pipeline jeden Spielraum zum Hochskalieren -- `k` faellt unter 1, und der Effekt duennerer Pfosten wird von der erzwungenen Verkleinerung wieder aufgefressen. Ein Rahmen mit gleichmaessigem Rand auf allen vier Seiten (wie jedes andere Asset im Katalog) laesst der Pipeline diesen Spielraum, und im ueblichen Fall (Rahmen-Seitenverhaeltnis naeher an 1 als an der Bildschirmdiagonale) wird die Bildbreite zur bindenden Groesse -- dort kuerzt sich `frame.w` aus der Rechnung komplett heraus, nur noch die Pfostenduenne (Nische-zu-Rahmen-Verhaeltnis) zaehlt. Deshalb steht in v7 wieder "generous even padding on all four sides" statt "flush against the very top/bottom edge".

Gemessen, nicht geschaetzt: v4 kam bei 280 CSS-Pixel Regalbreite heraus (Nische zu Rahmen 69 %, Hoehe 91 %), v7 bei 316 (85 %, 91 %) -- ein Plus von 13 %, das erste Mal seit dem Stilwechsel, dass ein Nachrender tatsaechlich mehr Platz bringt statt weniger.

**bgl_niche_frame (v7):**
Overhead photograph of a real hand-cut washi paper collage of a tall freestanding display cabinet, a physical artwork made of exactly four separate strips of Japanese kozo paper cut with a blade and stacked to form a single flat frame outline, two slender vertical side posts and one slender horizontal top rail and one slender horizontal bottom rail, only one strip per side with nothing doubled, nested, or mitred behind it -- a plain flat outline, never a picture-frame mat or bevel construction and never two frames nested inside each other, each strip no wider than one fifteenth of the cabinet's own overall width, the strips overlapping at the four corners with a crisp hairline shadow where one lies over the next, the open inner cavity dominating the composition and showing the plain white background straight through it, the cavity at least 90% of the cabinet's own width and at least 90% of its own height so the frame reads as a thin outline sketch and never as a solid block of wood, the bottom rail is a single straight horizontal strip and the absolute lowest point of the object with nothing extending below it -- no feet, no legs, no dangling tabs, no splayed extensions past any of the four strips, absolutely no back panel and no shelves and no crossbars and no centre post and no doors inside the cavity and no shadow cast into it, the paper a warm aged cedar brown of medium tone roughly one third as bright as fresh white paper and never pale, one small brushed brass gold paper accent at each of the four corners no larger than the frame strip itself, every cut edge showing the tiny nicks and slight wobble of a real hand-held knife and never a perfectly smooth machine line, visible long kozo fibres and gentle mottling in every sheet, absolutely no painted shading and no gradient within a sheet, no white or near-white areas anywhere on the cabinet itself, single isolated object, lit by soft even frontal light with no shadow falling onto the background, shot straight down and perfectly square to the camera, centered on a portrait 9:16 canvas with generous even padding on all four sides, pure solid white background (#FFFFFF), sharp silhouette edge definition, 8k resolution --style raw

**Negativ fuer v7:** double frame, nested frame, picture frame mat, beveled inset, mitred inner border, rabbet joint, layered border, second inner frame outline, 3d depth bevel, feet, legs, dangling tabs, splayed extensions, protruding elements below the frame, uneven bottom edge, thick frame, wide border, chunky posts, heavy solid wood, massive frame, bulky corner brackets, large brass ornament, plus das v4-Item-Negativ.

**Offen:** v7 hat noch eine sehr feine zweite Akzentlinie parallel innerhalb jedes Stegs -- kein Spalt, kein durchscheinender Hintergrund wie bei v6, nur eine Linie *innerhalb* eines durchgehenden Stegs. Fuer die Nischenvermessung irrelevant, optisch ein kleiner Rest gegenueber der angefragten reinen Einzellinie. Kein weiterer Versuch geplant, das Risiko einer schlechteren Zufallsvariante (siehe v6) wiegt schwerer als der kosmetische Gewinn.

**Header folgt der gemessenen Nische, nicht mehr einem festen Randabstand.** SCORE/MOVES sassen bisher auf `pad` von der Canvaskante, unabhaengig davon, wo der Rahmenpfosten tatsaechlich liegt. Bei v4s breiterem Rahmen fiel das nicht auf, bei v7s schmalerem lag der Pfosten genau dort, wo der Text saemtlicher Vorgaenger-Header stand. `UIScene.create` liest jetzt `GameScene.cavityWidth`/`cavityCenterX` (dafuer oeffentlich statt privat) und schiebt die Aussenspalten so weit nach aussen, bis sie den gemessenen Nischenrand um die tatsaechliche Textbreite (nicht eine angenommene Boxbreite) plus Sicherheitsabstand verfehlen -- gedeckelt an der Canvaskante. Der Rahmen-Halo-Effekt haelt den Text danach so oder so lesbar, aber ueber massivem Holz statt genau auf der Kante zum Hohlraum liest er sich sauberer.
...
### Vollbild-Layout (v3) — ersetzt durch v4

Dieser Plan ging von einem hinoki-farbenen Moebel vor einer Gartenszene (`bg_kissa_garden`) aus. Beides wurde verworfen, als der Stilwechsel zu "Yoru no Kissa" kam: `bgl_niche_frame` und `shelf_wood` sind jetzt Kiri-e-Papier (Abschnitt "Rahmen und Regalbrett v4"), der Hintergrund ist die gedruckte Nachtwand `bg_kissa_room` aus Reve (Abschnitt "Hintergrund fuer Reve"). Die Kernidee -- das Moebel bringt sein eigenes Seitenverhaeltnis mit, randlos statt zwischen Header und Booster eingepasst -- ist in v4 unveraendert uebernommen und im Code umgesetzt (siehe "Das Regalgehaeuse").
...
### Parallax-Hintergrund "Engawa-Garten" (bgl_ Layer) — stillgelegt

**Der Code, der diese Ebenen zeichnet, wurde mit dem Stilwechsel zu "Yoru no Kissa" entfernt** (`BG_LAYERS`, `createBgLayers` in `src/main.ts`). Tagsonne, Katze, Shiba, rosa Laternen und blauer Himmel waren genau die Kinderoptik-Signale, die der neue Stil abloesen sollte -- vor der gedruckten Nachtwand (`bg_kissa_room`) wuerden sie ohnehin stilistisch nicht mehr passen. Die Prompts und Renders bleiben unten als Referenz stehen, falls ein Tagesmodus zurueckkommt; die erzeugten WebP-Dateien liegen noch in `public/assets/items/`, werden aber von keinem Ladeaufruf mehr referenziert.

Statt eines einzelnen Bildes wurde der Hintergrund aus freigestellten Ebenen gebaut, damit sich Wolken, Laternen und Tiere unabhängig bewegen können. Die Szene: die Regalnische steht auf einer Engawa-Veranda, dahinter öffnet sich ein Garten — blauer Himmel, ferne Hügel, Wiese, oben hängende Chōchin-Laternen unter dem Dachvorsprung, unten spielen Shiba und Katze.
...
Das Loch ist danach die exakte lichte Nische und wird als `BG_CAVITY_RECTS` exportiert — präziser als die Helligkeits-Heuristik, weil es direkt aus dem Alphakanal kommt. Zusätzlich geht die äußere Kontur als `BG_FRAME_RECTS` mit.

**Skalierung ab v4: Cover statt Einpassen.** Der v4-Rahmen bringt sein Seitenverhältnis selbst mit (Ober- und Unterkante sind im Render an den Canvasrand gezogen), deshalb füllt `k = min(height / (frame.h * source.height), width / (frame.w * source.width))` die volle Canvashöhe, statt wie in v2/v3 zwischen `78 * uiScale` und `height - 74 * uiScale` einzupassen. Header und Booster-Reihe liegen jetzt *über* dem Rahmen statt in eigenen Bändern darüber und darunter — das ist der Randlos-Effekt, den die zwei Spalten an Fläche brauchen. Der Bodenschatten aus v2/v3 ("erdet das Möbel auf der Wiese") ist mit ihm entfallen: es gibt keine Wiese mehr, auf der das Möbel steht, die Unterkante sitzt an der Canvaskante.

**Kein Shoji-Panel mehr.** Bis v3 lag zwischen Nische und Garten ein halbtransparentes Rechteck (Deckkraft 0.55), weil Himmel und Hügel sonst keinen Kontrast zu den Goods boten. Die offene Nische zeigt jetzt direkt die gedruckte Nachtwand `bg_kissa_room` — die liefert den Kontrast durch ihre eigene Dunkelheit (Zielwerte siehe "Hintergrund für Reve"), ein zusätzliches Panel bräuchte es nicht und würde das Papier der Wand nur verwaschen.

**Zwei Spalten statt einer.** `buildLevel` in `GameScene` teilt die Nischenbreite in zwei Bretter mit `columnGap` dazwischen (`shelfWidth = (cavityWidth - columnGap) / 2`), die Zeilenzahl ist `Math.ceil(shelfCount / 2)`. Bei ungerader Brettanzahl steht das letzte Brett mittig statt links, sonst hinge es sichtbar neben einer leeren rechten Hälfte. Die alte Klammer `clamp(shelfWidth / 3, 78, 104)` in der `Shelf`-Konstruktion war auf ein einzelnes breites Brett kalibriert und ist raus — bei halbierter Brettbreite wäre sie enger als das Brett selbst gewesen und hätte Slots über den Rand hinaus geschoben. `SHELF_CAVITY_FILL` (der Faktor, der die Messingstifte an den Rahmen schob) ist mit den Stiften selbst gegenstandslos geworden, siehe "Rahmen und Regalbrett v4".

**Goods duerfen ihren Slot nicht ueberragen.** `getLayoutScale` kennt nur die Canvasbreite, nicht die tatsaechliche Brettbreite -- bei zwei Spalten ist der Slot-Abstand (`shelfWidth / 3`) oft enger, als dieser Massstab annimmt. Ohne Deckel ragte ein grossformatiges Item wie `dango_stick` (Groessenfaktor 1.25 aus der Deckungs-Normalisierung) sichtbar in den Nachbarslot -- am deutlichsten bei den Toast-Wuerfeln, die Kante an Kante standen. `buildLevel` deckelt `itemScale` deshalb zusaetzlich: `maxItemScale = (slotSpacing * SLOT_FILL) / (ITEM_SIZE * ITEM_SIZE_MAX)`, `SLOT_FILL = 0.86` laesst rundum Luft. Der Deckel gilt fuer das ganze Spiel gleich (`cavityWidth` ist levelunabhaengig), Goods bleiben also ueberall gleich gross -- kein Item wird pro Regal einzeln verkleinert.

Bei diesem Rahmen-Render zieht der Deckel deutlich: `slotSpacing` liegt bei rund 45 CSS-Pixeln statt der in der Layout-Planung angenommenen 61, weil die Nische nur 68 % der Rahmenbreite offen ist statt der im Prompt verlangten mindestens 90 % (siehe "Rahmen und Regalbrett v4"). Ein Nachrender mit schmalerem Pfosten wuerde `slotSpacing` automatisch vergroessern und die Goods ohne Codeaenderung wieder anwachsen lassen.

**Der Sitz am Rahmen wurde pixelgenau nachgemessen, nicht nur angenommen:** Regalbrett und Rahmenpfosten beruehren sich exakt (Abweichung < 5 px bei 1236 px Canvasbreite), die Kavitaetsmessung ist korrekt. Was wie eine Luecke aussieht, ist der ungewoehnlich breite Pfosten aus genau diesem Grund -- kein Positionierungsfehler.

Die alte Fallback-Bemessung (`bg_kissa_niche` ohne gemessenes `BG_CAVITY_RECTS`) bleibt als Notanker bestehen, falls `bgl_niche_frame` einmal fehlt — auch dort stehen weiterhin zwei Spalten, nur die Zeilenhöhe kommt dann aus den festen Design-Werten in `verticalScale` statt aus der gemessenen Nischenhöhe.
